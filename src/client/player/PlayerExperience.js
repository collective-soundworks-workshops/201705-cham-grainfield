...
class PlayerExperience extends soundworks.Experience {
  ...
  start() {
    ...
    this.recordId = `record-${client.index % 4}`;
    ...
    // gain for each group
    this.sharedParams.addParamListener(`${this.recordId}-gain`, (value) => {
  ...
  startState() {
    ...
    this.sharedRecorder.addListener(this.recordId, [], this._onBuffer);
    this.view.setState('starting');
  }

  endState() {
    this.sharedRecorder.removeListener(this.recordId);
  ...
  _onBuffer(buffer, phase) {
    console.log('new buffer', this.recordId);
